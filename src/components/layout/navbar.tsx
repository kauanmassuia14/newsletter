...
export function Navbar({ variant = "landing" }: NavbarProps) {
    ...
    const isAdminEmail = user?.email === "kauanmassuia14@gmail.com";
    ...
    return (
        <nav className={`${isLanding ? "fixed" : "sticky"} inset-x-0 top-0 z-50 border-b border-slate-200/60 bg-white/80 backdrop-blur-2xl dark:border-white/[.06] dark:bg-brand-900/80`}>
            <div className={`mx-auto flex items-center justify-between px-4 py-3 sm:px-6 ${isFeed ? "max-w-[620px]" : "max-w-7xl"}`}>
                <Link href={user ? "/feed" : "/"} className="group flex items-center gap-2.5">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {isLanding && !user && (
                        <div className="flex items-center gap-2">
                            ...
                            <Link href="/register">
                                <Button size="sm">Começar Gratis</Button>
                            </Link>
                        </div>
                    )}

                    {(isFeed || (isLanding && user)) && (
                        <div className="flex items-center gap-2 sm:gap-3">
                            ...
                            <Link href="/settings" className="rounded-xl p-2 text-slate-400 transition hover:bg-slate-100 dark:hover:bg-white/[.05]" title="Configurações">
                                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M10.343 3.94c.09-.542.56-.94 1.11-.94h1.093c.55 0 1.02.398 1.11.94l.149.894c.07.424.384.764.78.93.398.164.855.142 1.205-.108l.737-.527a1.125 1.125 0 011.45.12l.773.774a1.125 1.125 0 01.12 1.45l-.527.737c-.25.35-.272.806-.107 1.204.165.397.505.71.93.78l.894.15c.542.09.94.56.94 1.109v1.094c0 .55-.398 1.02-.94 1.11l-.894.149c-.424.07-.764.383-.929.78-.165.398-.143.854.107 1.204l.527.738a1.125 1.125 0 01-.12 1.45l-.772.773a1.125 1.125 0 01-1.45.12l-.737-.527c-.35-.25-.806-.272-1.203-.107-.397.165-.71.505-.781.929l-.149.894c-.09.542-.56.94-1.11.94h-1.094c-.55 0-1.019-.398-1.11-.94l-.148-.894c-.071-.424-.384-.764-.781-.93-.398-.164-.854-.142-1.204.108l-.738.527a1.125 1.125 0 01-1.45-.12l-.773-.774a1.125 1.125 0 01-.12-1.45l.527-.737c.25-.35.273-.806.108-1.204-.165-.397-.505-.71-.93-.78l-.894-.15c-.542-.09-.94-.56-.94-1.109v-1.094c0-.55.398-1.02.94-1.11l.894-.149c.424-.07.765-.383.93-.78.165-.398.143-.854-.108-1.204l-.526-.738a1.125 1.125 0 01.12-1.45l.773-.773a1.125 1.125 0 011.45-.12l.737.527c.35.25.807.272 1.204.107.397-.165.71-.505.78-.929l.15-.894z" />
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                </svg>
                            </Link>
                            <button onClick={handleLogout} className="rounded-xl p-2 text-slate-400 transition hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-500/10 cursor-pointer" title="Sair">
                            ...
                            <div className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-brand text-[11px] font-bold text-white shadow-sm dark:bg-accent-400/20 dark:text-accent-300">
                                {profile?.avatar_url ? (
