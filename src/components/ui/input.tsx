...
export function Input({
    ...
    return (
        <div className={className}>
            ...
            focus:border-brand focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand/10
            dark:border-white/10 dark:bg-white/[.04] dark:text-white dark:placeholder-slate-600
            dark:focus:border-accent-400 dark:focus:bg-white/[.06] dark:focus:ring-accent-400/10
            ${prefixIcon ? "pl-10" : ""}
            ${isPassword || suffixIcon ? "pr-10" : ""}
            ${error ? "border-red-300 focus:border-red-500 focus:ring-red-500/10 dark:border-red-500/50" : ""}`}
                    {...props}
                />
                {isPassword && (
                    <button
                        ...
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-brand cursor-pointer dark:hover:text-white"
                    >
                        {showPw ? (
